...
if __name__ == '__main__':
    ...
    try:
        with get_sys_db() as db:
            client = ClientFactory.get_client_by_id(11)
            ...
            #     logger.info(session)
            # path = client.get_resource_manager().get_wx_owner_img()
            # logger.info(path)
            ...
            # chat_room_manager = client.get_chat_room_manager()
            # info = chat_room_manager.chatroom_info('24902414430@chatroom')
            # for member in info.members:
            #     logger.info(f"nickname is : {member.nickname}")
            query = SingleMsgFilterObj(
                username='wxid_wv8qg9krjmdb22',
                db_name='decoded_message_0.db',
                local_id=35
            )
            message_manager = client.get_message_manager()
            message_manager.message(query)
